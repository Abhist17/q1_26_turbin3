...
import { VaultSolana } from "../target/types/vault_solana";
import { expect } from "chai";

describe("vault_solana", () => {
  // Configure the client to use the local cluster.
  const provider = anchor.AnchorProvider.env();
  anchor.setProvider(provider);

  const program = anchor.workspace.vault_solana as Program<VaultSolana>;
  ...
  before(async () => {
    // Airdrop for fees and confirm properly
  ...
  it("Deposit SOL into the vault", async () => {
    ...
    // User balance decreases by amount + fees (flexible check)
    const totalCost = initialUserBalance - finalUserBalance;
    expect(totalCost).to.be.greaterThan(depositAmount);
    expect(totalCost).to.be.lessThan(depositAmount + 10000);
  });
  ...
  it("Withdraw SOL from the vault", async () => {
    ...
    // User balance increases by amount - fees (flexible check)
    const netGain = finalUserBalance - initialUserBalance;
    expect(netGain).to.be.greaterThan(withdrawAmount - 10000);
    expect(netGain).to.be.lessThan(withdrawAmount);
  });
  ...
  it("Close the vault", async () => {
    ...
    // User gets back the remaining balance - fees (flexible check)
    const expectedReturn = initialVaultBalance + initialVaultStateBalance;
    const netReturn = finalUserBalance - initialUserBalance;
    expect(netReturn).to.be.greaterThan(expectedReturn - 10000);
    expect(netReturn).to.be.lessThanOrEqual(expectedReturn);
  });
});
